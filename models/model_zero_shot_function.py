...
def prompt_zero_shot(language:str,test_data:pd.DataFrame,i:int)->str:
...
def classification_n_shot(pipe,message:list,tokenizer:AutoTokenizer)->list:
    ...
    if predicted_labels.startswith("[" ) and predicted_labels.endswith("]"):
        print(f"Predicted labels before removing brackets and splitting: {predicted_labels}")
...
def zero_shot_classification(file_name:str,model_name:str,language:str="en"):
    ...
    for i in range(len(test_data)):
        ...
        prompt = prompt_zero_shot(language=language, test_data=test_data, i=i)
    ...
    evaluate_model(y_true, predicted_labels, model_name=model_name.split("/")[-1] + "_" + language + "_model_" + "one_zero_shot")
